...
class VGGSfMRunner:
    ...
    def sparse_reconstruct(
        ...
        #     extrinsics_opencv = self.rename_colmap_recons_and_rescale_camera(
        #         extrinsics_opencv, # [1, S, 3, 4]
        #         image_paths,
        #         crop_params,
        #         img_size,
        #         shared_camera=self.cfg.shared_camera,
        ...
        assert extrinsics_opencv.shape[0] == self.cfg.batch_size, f"batch size {extrinsics_opencv.shape[0]} is inconsistent with {self.cfg.batch_size}."
    ...
    def rename_colmap_recons_and_rescale_camera(
        ...
        for idx in range(len(extrinsics.shape[0])):
            # Reshaped the padded&resized image to the original size
            # Rename the images to the original names
            # pyimage = reconstruction.images[pyimageid]
            # pycamera = reconstruction.cameras[pyimage.camera_id]
            # pyimage.name = image_paths[pyimageid]
            extrinsic = extrinsics[idx] # [3, 4]

            if rescale_camera:
                # Rescale the camera parameters
                real_image_size = crop_params[0, idx][:2]

                bbx_top_left = crop_params[0, idx][4:6]
